...
function AnalysisSourceBadge({ narrativePending, analysisQuality }) {
  if (narrativePending) {
    return (
      <div className="flex items-start gap-3 p-4 bg-amber-50 ring-1 ring-amber-200 rounded-2xl">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-amber-800">
            Python scores only — LLM narrative unavailable
          </p>
          <p className="text-xs text-amber-600 mt-0.5 leading-relaxed">
            Scores, skill gaps, and timeline are accurate (computed by Python). 
            Strengths, weaknesses, and interview questions are auto-generated templates 
            because Ollama is offline or timed out.
          </p>
          <p className="text-xs text-amber-500 mt-1.5 font-medium flex items-center gap-1">
            <RefreshCw className="w-3 h-3" />
            Start the Ollama service and re-analyse to get AI-written narrative.
          </p>
        </div>
        {analysisQuality && (
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-green-50 ring-1 ring-green-200 rounded-2xl">
      ...
      <p className="text-xs font-semibold text-green-700 flex-1">
        AI-powered analysis — scores + narrative generated by ARIA
      </p>
      {analysisQuality && (
        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ring-1 shrink-0 ${
          analysisQuality === 'high'   ? 'bg-green-100 text-green-700 ring-green-200' :
  ...
  return (
    <>
      <div className="bg-white/90 backdrop-blur-md rounded-3xl ring-1 ring-brand-100 shadow-brand p-6 md:p-8 space-y-6">
        ...
        {/* Analysis source badge — always visible so users know which path ran */}
        {!isPending && (
          <AnalysisSourceBadge
            narrativePending={narrative_pending}
