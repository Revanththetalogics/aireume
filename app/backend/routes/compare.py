...
log = logging.getLogger("aria.compare")


def _scalar_experience_match(val):
    """Extract scalar score from experience_match (handles dict and legacy int formats)."""
    if isinstance(val, dict):
        return val.get("score", 0) or 0
...
def compare_candidates(
        ...
        for r in results:
            def _safe_loads(data, field="data"):
                try:
                    ...
                    log.warning("Malformed JSON in %s for result id=%s", field, r.id)
                    return {}
            ...
            # Normalize experience_match to scalar for frontend rendering & max() comparison
            exp_match_raw = score_breakdown.get("experience_match", 0)
            exp_match_scalar = _scalar_experience_match(exp_match_raw)
            score_breakdown["experience_match"] = exp_match_scalar

            # Normalize skill_match to always be a dict with "score" key
            sm = score_breakdown.get("skill_match", 0)
            if isinstance(sm, int):
                score_breakdown["skill_match"] = {"score": sm, "confidence_weighted": False, "avg_confidence": 1.0}
    ...
    try:
        if comparison:
            ...
            for c in comparison:
                c["winners"] = {
                    "overall":    c["fit_score"] == max_fit and max_fit > 0,
                    "skills":     _sm_score(c["score_breakdown"]) == max_skill and max_skill > 0,
                    "experience": c["score_breakdown"].get("experience_match", 0) == max_exp and max_exp > 0,
