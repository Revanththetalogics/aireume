...
- Services encapsulate parsing, gap detection, hybrid pipeline scoring, and LLM orchestration.
- Routes expose endpoints for analysis, candidate listing/detail, history, export, and comparison.
...
- ScreeningResult persists analysis outcomes and links to candidates and role templates.
...
- Hybrid pipeline orchestrates Python-based scoring and LLM narrative.
- Deduplication logic identifies duplicates across email, file hash, and name+phone.
- Routes expose endpoints for single and batch analysis, candidate listing/detail, history, export, and comparison.
...
The system integrates parsing, gap detection, hybrid scoring, and persistence. Deduplication ensures candidate identity is preserved across uploads and re-analyses. Profiles are stored for fast re-analysis and auditability.
...
Client->>Route : POST /api/analyze (resume, job_description)
Route->>Parser : parse_resume()
Parser-->>Route : parsed_data
Route->>Gap : analyze_gaps(work_experience)
Gap-->>Route : gap_analysis
Route->>Pipe : run_hybrid_pipeline(parsed_data, gap_analysis, jd)
Pipe-->>Route : analysis_result
...
### Resume Parsing and Extraction Workflows
- ResumeParser supports PDF, DOCX, DOC, TXT, RTF, HTML, ODT, and plain text.
- Extracts raw text, work experience, skills, education, and contact info.
- Enriches parsed output with name inference from email or relaxed heuristics.
- Uses PyMuPDF with pdfplumber fallback for PDFs; normalizes Unicode.
...
PDF --> |No| DOCX{"DOCX?"}
DOCX --> |Yes| DOCXRead["Read paragraphs + tables"]
DOCX --> |No| TXT{"TXT?"}
...
Route->>HP : run_hybrid_pipeline(resume_text, job_description, parsed_data, gap_analysis, jd_analysis)
HP-->>Route : result (scores, matched/missing skills, breakdown)
Route->>SR : persist ScreeningResult
SR-->>Route : result_id
...
- After parsing and gap analysis, the hybrid pipeline produces a result.
- The route persists both the ScreeningResult and updates the Candidate profile snapshot.
...
Client->>Route : POST /api/analyze/batch (resumes, job_description)
Route->>DB : persist ScreeningResults
DB-->>Route : result_ids
Route-->>Client : ranked batch results
Client->>Export : GET /api/export/csv?ids=
Export->>DB : fetch results
DB-->>Export : rows
...
## Dependency Analysis
The following diagram shows key dependencies among modules involved in candidate management.

```mermaid
graph LR
PS["parser_service.py"] --> HP["hybrid_pipeline.py"]
GD["gap_detector.py"] --> HP
AS["analysis_service.py"] --> HP
RA["routes/analyze.py"] --> PS
RA --> GD
RA --> HP
...
## Conclusion
The candidate management system integrates robust parsing, deduplication, and analysis workflows with durable storage and auditability. It supports efficient re-analysis, bulk operations, and export for downstream ATS use. Extensibility is provided through model additions, parser customization, and skills registry updates. Privacy and lifecycle management should be considered for production deployments.
